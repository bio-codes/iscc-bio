...
def extract_2d_slices_for_iscc(bio_img: BioImage, max_slices: int = 5) -> List[Tuple[str, np.ndarray]]:
    """
    ...
    try:
        ...
        if 'C' in dims:
            ...
            for c in range(min(max_slices, num_channels)):
                ...
                print(f"    Channel {c}: {slice_2d.shape}, range={slice_2d.min()}-{slice_2d.max()}")
        ...
        # Get Z-stack slices if available
            ...
            for z in z_indices:
                ...
                print(f"    Z-slice {z}: {slice_2d.shape}, range={slice_2d.min()}-{slice_2d.max()}")

        # Single 2D image
        else:
            yx_data = bio_img.get_image_data("YX")
            slices.append(("Single_2D", yx_data))
            print(f"    Single 2D: {yx_data.shape}, range={yx_data.min()}-{yx_data.max()}")
...
def analyze_iscc_requirements(shape: Tuple, dims: str, slices: List[Tuple[str, np.ndarray]]) -> Dict[str, Any]:
    """
    ...
    if 'C' in dims and 'Z' in dims:
        ...
        analysis['processing_strategy'] = 'Multi-channel Z-stack: process each channel*Z combination'
        ...
        analysis['processing_strategy'] = 'Multi-channel: process each channel separately'
    ...
    for filename in large_files:
        ...
        if file_path.exists():
            try:
                ...
                if hasattr(bio_img, 'channel_names') and bio_img.channel_names:
                    print(f"  Channels: {[str(name) for name in bio_img.channel_names]}")
