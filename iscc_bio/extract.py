"""Bioimage feature extraction and thumbnail generation."""
...
def extract_thumbnail(
    ...
    """
    Extract a thumbnail from a bioimage file using lazy loading.
    ...
    Args:
        ...
        output_path: Optional output path for thumbnail.
                    If None, saves as <original>.thumb.png
    ...
    if output_path is None:
        output_path = image_path.with_suffix(image_path.suffix + ".thumb.png")

    logger.info(f"Extracting thumbnail from {image_path.name}")
    ...
    logger.info(f"Image dimensions: {dict(zip(dims.order, shape))}")

    # Check for available resolution levels and use lowest for quick thumbnail
    resolution_levels = img.resolution_levels
    logger.info(f"Available resolution levels: {resolution_levels}")
    ...
    if y_size > 2000 or x_size > 2000:
        ...
        logger.info(f"Downsampling with stride Y:{y_stride}, X:{x_stride}")
    ...
    logger.info(f"Thumbnail extracted in {elapsed:.2f} seconds: {output_path}")
...
def extract_scenes(
    ...
    for scene_idx in range(num_scenes):
        ...
        logger.info(f"{scene_name} extracted in {scene_elapsed:.2f} seconds: {output_path}")
